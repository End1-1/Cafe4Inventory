...
class AragamashScreen extends AppScreen {

  AragamashScreen({super.key, String itemQr = ''}) : super(model: AragamashModel()){
    (model as AragamashModel).itemQr = itemQr;
    model.refresh();
  }
    ...
    return StreamBuilder(stream: model.controller.stream, builder: (builder, snapshot) {
      ...
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
        TextFormField(
          ...
          onChanged: (s) {
            (model as AragamashModel).filter(s);
          },),
              ...
              Column(
                  ...
                  children: [for (final bi in snapshot.data) ...[
                    Row(children: [
                      ...
                      Container(margin: const EdgeInsets.fromLTRB(5, 5, 5, 10), width: 50, child: Text(bi.ffadepartment),),
                    ...
                    Row(children: [
                      ...
                      Container(margin: const EdgeInsets.fromLTRB(5, 5, 5, 10), width: 50, child: Text("${(model as AragamashModel).dataQty(snapshot.data)}"),),
                    ],),]),
            )))]);
    });
  }
      ...
      InkWell(
          onTap: () {
            DlgIndex().getData(context, 'Պահեստ', HttpQuery.rAsStorageList).then((value) {
              if (value != null) {
                (model as AragamashModel).store = value.id;
                model.refresh();
              }
            });
          },
