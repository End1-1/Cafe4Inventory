...
class AppWebSocket {
  ...
  void _connect() {
    ...
    try {
      ...
      sendMessage(jsonEncode({'command':'register_socket', 'socket_type':4, 'userid':0, 'database': 'nodb'}), null);
    } catch (e) {
      _replyMessage({'errorCode':-10, 'errorMessage': e.toString()});
      _closeChannel();
      _setConnectionState(ConnectionStateType.error);
  ...
  void sendMessage(String msg, Function(Map<String, dynamic>)? handle) {
    ...
    try {
      final a = jsonDecode(msg);
      a['command'] = 'cafe4inventory';
      a['handler'] = 'inventory';
      a['workerDb'] = prefs.fbDb();
      _channel?.sink.add(jsonEncode(a));
    } catch (e) {
      _setConnectionState(ConnectionStateType.error);
      _socketDisconnected();
